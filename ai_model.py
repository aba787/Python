            ...
            for i in range(n_samples):
                ...
                # DoS attacks (15%)
                    ...
                    features = [
                        ...
                        np.random.uniform(0.8, 1.0),  # serror_rate (high)
                        np.random.uniform(0.8, 1.0),  # srv_serror_rate (high)
                        0, 0, 0, 0, 0,
                        np.random.poisson(255),  # dst_host_count (max)
                        np.random.poisson(255),  # dst_host_srv_count
                        0, 0, 0, 0,
                        np.random.uniform(0.8, 1.0),  # dst_host_serror_rate
                        np.random.uniform(0.8, 1.0),  # dst_host_srv_serror_rate
                        0, 0
                    ]
                    data.append(features)
                    labels.append('dos')
                ...
                # Probe attacks (10%)
                    ...
                    features = [
                        ...
                        1,  # srv_count (low)
                        np.random.uniform(0.8, 1.0),  # serror_rate
                        np.random.uniform(0.8, 1.0),  # srv_serror_rate
                        0, 0, 0, 0, 0,
                        np.random.poisson(255),  # dst_host_count
                        1,  # dst_host_srv_count
                        0,  # dst_host_same_srv_rate
                        np.random.uniform(0.8, 1.0),  # dst_host_diff_srv_rate
                        0, 0,
                        np.random.uniform(0.8, 1.0),  # dst_host_serror_rate
                        np.random.uniform(0.8, 1.0),  # dst_host_srv_serror_rate
                        0, 0
                ...
                # U2R attacks (2%)
                    ...
                    features = [
                        ...
                        np.random.uniform(0.5, 0.8),  # dst_host_same_srv_rate
                        np.random.uniform(0.2, 0.5),  # dst_host_diff_srv_rate
                        0, 0, 0, 0, 0, 0
                    ]
                    data.append(features)
                    labels.append('u2r')
    ...
    def extract_features_from_packet(self, packet_data):
        ...
        if packet_type == 'dos_simulation':
            ...
            features = [
                ...
                random.poisson(1),  # hot
                0,  # num_failed_logins
                1,  # logged_in
                0, 0, 0, 0,  # num_compromised, root_shell, su_attempted, num_root
                random.poisson(0.1),  # num_file_creations
                0,  # num_shells
                random.poisson(0.1),  # num_access_files
                0, 0, 0,  # num_outbound_cmds, is_host_login, is_guest_login
                random.poisson(10),   # count
                random.poisson(5),    # srv_count
                random.uniform(0, 0.1),  # serror_rate
                random.uniform(0, 0.1),  # srv_serror_rate
                random.uniform(0, 0.1),  # rerror_rate
                random.uniform(0, 0.1),  # srv_rerror_rate
                random.uniform(0.8, 1.0),  # same_srv_rate
                random.uniform(0, 0.2),    # diff_srv_rate
                random.uniform(0, 0.2),    # srv_diff_host_rate
                random.poisson(100),       # dst_host_count
                random.poisson(50),        # dst_host_srv_count
                random.uniform(0.8, 1.0),  # dst_host_same_srv_rate
                random.uniform(0, 0.2),    # dst_host_diff_srv_rate
                random.uniform(0, 0.2),    # dst_host_same_src_port_rate
                random.uniform(0, 0.2),    # dst_host_srv_diff_host_rate
                random.uniform(0, 0.1),    # dst_host_serror_rate
                random.uniform(0, 0.1),    # dst_host_srv_serror_rate
                random.uniform(0, 0.1),    # dst_host_rerror_rate
                random.uniform(0, 0.1),    # dst_host_srv_rerror_rate
            ]
